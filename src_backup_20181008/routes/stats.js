  ...
  dbConnection.query(`SELECT count(DISTINCT idcontratto) as totaleContratti, count(DISTINCT codice_da_url) as totaleCodiciDaUrl FROM pagamento_online_idcontratto_cf WHERE mandato = ${dbConnection.escape(session.stats.mandato)} AND idcliente = ${dbConnection.escape(session.stats.cliente)}`)
    ...
    .then(
      (data) => {
        ...
        return dbConnection.query(`SELECT DISTINCT codice_da_url as codice FROM pagamento_online_idcontratto_cf WHERE mandato = ${dbConnection.escape(session.stats.mandato)}  AND idcliente = ${dbConnection.escape(session.stats.cliente)}`)
          .then(
            (results) => {
          ...
          .then(
            (results) => {
              ...
              return Promise.resolve({
                ...
                consumedContratti: totalConsumedContratti > 0 ? results.map((r) => {
                  if (!r.idcontratto) {
