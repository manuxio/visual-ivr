    ...
    .then(
      (fullConfig) => {
        ...
        .query(`UPDATE onlinePaymentTransactions SET
          ...
          completed = 1,
          status = ${req.dbConnection.escape(status)},
          remotestatus = ${req.dbConnection.escape(stripslashes(remotestatus || null))},
          body = ${req.dbConnection.escape(JSON.stringify(req.body))},
          updatetime = NOW()
          WHERE ID = ${req.dbConnection.escape(addInfo4)}
        `)
    ...
    .then(
      (results) => {
        if (!results || results.length !== 1) {
          ...
          logger(req, `Unable to validate addInfo4 in check transaction, ${addInfo4}, possible duplicate transaction notification`);
